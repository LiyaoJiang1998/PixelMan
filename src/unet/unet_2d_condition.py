...
class CustomUNet2DConditionModel(UNet2DConditionModel):
    ...
    def __init__(
        ...
        self.time_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim, cond_proj_dim=time_cond_proj_dim)

        # class embedding
        if class_embed_type is None and num_class_embeds is not None:
            self.class_embedding = nn.Embedding(num_class_embeds, time_embed_dim)
        elif class_embed_type == "timestep":
            self.class_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim, cond_proj_dim=time_cond_proj_dim)
    ...
    def forward(
        ...
        sample = self.mid_block(
            ...
            iter_cur = iter_cur, save_kv=save_kv, mode=mode, mask=mask, sa_masking_ipt=sa_masking_ipt,
        )
