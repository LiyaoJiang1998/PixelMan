...
import numpy as np
from src.unet.unet_2d_condition import CustomUNet2DConditionModel
...
from src.utils.attn_utils import AttentionStore

class EditPipeline:
    def __init__(self, sd_id='runwayml/stable-diffusion-v1-5', ip_id='models/ip_sd15_64.bin', NUM_DDIM_STEPS=50, precision=torch.float32, ip_scale=0, use_ip_adapter=False):
        unet = CustomUNet2DConditionModel.from_pretrained(sd_id, subfolder="unet", torch_dtype=precision)
        ...
        onestep_pipe.scheduler = scheduler
    ...
    def load_adapter(self, model_path, scale=1.0, 
                     use_attnstore=True):
        # Saving SA and CA maps, create AttentionStore
        attention_to_use = [
                            'down_blocks.2.attentions.0.transformer_blocks.0.attn2', # CA down2-0 16x16
                            'down_blocks.2.attentions.1.transformer_blocks.0.attn2', # CA down2-1 16x16
                            "up_blocks.1.attentions.0.transformer_blocks.0.attn2", # CA up1-0 16x16
                            "up_blocks.1.attentions.1.transformer_blocks.0.attn2", # CA up1-1 16x16
                            "up_blocks.1.attentions.2.transformer_blocks.0.attn2", # CA up1-2 16x16
                            "up_blocks.3.attentions.0.transformer_blocks.0.attn1", # SA up3-0 64x64
                            "up_blocks.3.attentions.1.transformer_blocks.0.attn1", # SA up3-1 64x64
                            "up_blocks.3.attentions.2.transformer_blocks.0.attn1"] # SA up3-2 64x64
        
        device = self.pipe._execution_device
        if use_attnstore:
            self.pipe.attention_store = AttentionStore(attention_to_use, device)
        else:
            self.pipe.attention_store = None

        # Update the original AttentionProcesseors to call attention_store
        attn_procs = {}
        att_count = 0
        print(self.pipe.unet.attn_processors.keys())
        ...
        for name in self.pipe.unet.attn_processors.keys():
            ...
            module = self.pipe.unet.get_submodule(module_name)
            setattr(module, "attn_name", module_name)
                
            if cross_attention_dim is None:
                attn_procs[name] = AttnProcessor(attnstore=self.pipe.attention_store)
            else:
                attn_procs[name] = IPAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim,
                                                   scale=scale,num_tokens= self.num_tokens,
                                                   attnstore=self.pipe.attention_store).to('cuda', dtype=self.precision)
                
        self.pipe.unet.set_attn_processor(attn_procs)
        if self.pipe.attention_store:
            self.pipe.attention_store.num_att_layers = att_count
