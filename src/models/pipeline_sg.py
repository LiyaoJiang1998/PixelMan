...
from src.unet.attention_processor_sg import IPAttnProcessor, AttnProcessor, Resampler
from transformers import CLIPVisionModelWithProjection, CLIPImageProcessor
from src.models.Sampler_sg import Sampler

from src.utils.attn_utils import AttentionStore

class DragonPipeline:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1', ip_id='models/ip_sd15_64.bin', NUM_DDIM_STEPS=50, precision=torch.float32, ip_scale=0, use_ip_adapter=True):
    ...
    def load_adapter(self, model_path, scale=1.0, 
        ...
        if use_attnstore:
            self.pipe.attention_store = AttentionStore(attention_to_use, device, keep_grad=True)
        else:
